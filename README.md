...
Внутри репозитория `DaoMongo` реализовано четкое разделение обязанностей между документоориентированным движком базы данных и файловой системой GridFS. Это позволяет эффективно использовать оперативную память и ресурсы виртуальных потоков:

```mermaid
graph TD
    A[Драйвер MongoClient] --> B[getDatabase]
    A --> C[getGridFS]
...
### 1. Метод `getDatabase()` — Для игровых состояний (BSON)
Используется для работы со стандартными коллекциями MongoDB (в частности, `saved_games`).
* **Что хранит:** Структуру игрового стакана `char[][]`, очки и имя игрока, сериализованные через Google `Gson`.
* **Почему так:** Данные матча весят всего несколько Килобайт. Они идеально укладываются в концепцию единого атомарного документа, моментально обновляются через `replaceOne` с флагом `upsert(true)` и не фрагментируют диск.
...
1. **Концепция единого атомарного документа (Защита от сбоев)**
   В отличие от реляционных баз данных (SQL), где для сохранения ячеек стакана и счета игрока пришлось бы делать каскад мелких запросов `INSERT` в связанные таблицы, в MongoDB всё состояние матча (`SavedGame`) сериализуется через Google `Gson` в один монолитный BSON-документ. Драйвер гарантирует атомарность на уровне документа: он либо запишется целиком, либо не запишется вовсе. Ситуация, при которой из-за сбоя сети счет сохранился, а стакан пропал, физически невозможна.

2. **Исключение гонки данных (Атомарный Upsert)**
   Флаг `upsert(true)` (Update + Insert) избавляет Java-код от лишних проверок в стиле `if (играСуществует) { update() } else { insert() }`. Такой ручной подход в многопоточной среде неизбежно приводит к Race Condition, когда два параллельных запроса пытаются одновременно создать одну и ту же игру, порождая `Duplicate Key Exception`. Команда `replaceOne` с флагом `upsert(true)` выполняет проверку и замену на стороне MongoDB атомарно, мгновенно блокируя конкурентные запросы.

3. **Защита от фрагментации диска**
   Постоянное изменение и перезапись документов разного размера в MongoDB приводит к фрагментации файловой системы — база начинает двигать данные по жесткому диску, оставляя «дыры» и снижая скорость I/O. Однако структура стакана Тетриса (`char[][]`) имеет фиксированный и экстремально маленький размер (несколько Килобайт). При вызове `replaceOne` обновленный JSON идеально вписывается в те же физические байты на диске, которые занимал старый документ, сохраняя диск ровным, а скорость чтения — максимальной.


### 2. Метод `getGridFS()` — Для медиа-контента (GridFS)
Используется как виртуальная файловая система поверх коллекций `fs.files` (метаданные) и `fs.chunks` (бинарные куски).
* **Что хранит:** Тяжелые массивы байт (`byte[]`) — аватарки (`MugShot`) и снимки экранов матчей.
* **Почему так:** Вместо загрузки файла целиком в RAM, GridFS автоматически нарезает графику на фиксированные чанки размером 1 МБ (`1048576 bytes`). Это позволяет виртуальным потокам Java 21 читать и писать файлы потоками (`InputStream`/`OutputStream`) в режиме конвейера, полностью защищая микросервис от утечек памяти при массовой одновременной загрузке скриншотов.
